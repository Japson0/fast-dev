 ...
 */
public class CustomBaseController<P, T extends BaseEntity<Long>> extends BaseController<Long, T> {
     ...
     */
    @ApiOperation("分页查询")
    public ChartsResponse<GridData<PageConditionQuery<T>>> getPageForGrid(PageConditionQuery<P> conditionDTO, Class<?> clazz) {
    ...
    protected ChartsResponse<GridData<Collection<T>>> getAllForGrid(P condition, Class<?> clazz) {
    ...
    public RestResponse deleteBatch(@RequestParam List<Long> ids) {
        return RestResponse.renderSuccess(baseService.deleteById(ids));
    }
}
