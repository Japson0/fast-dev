...
- 做日志索引的别名。比如把所有日志的别名设置为 `log_all`,那么我们只要去检索索引`log_all`即可检索所有的日志索引，这有个问题，因为是把查询条件丢到所有的日志索引里面查，本质上就是遍历所有对应的索引。
- 根据一定规则查询对应的索引，比如要查询2023年6月份14日的索引，直接去log_2023_06_14的索引下去查
